...
// Date constructor, so these tests pass regardless of the machine's own
// timezone.
describe("getFinancialPeriod", () => {
  it("puts the 27th itself in a period that starts that same day", () => {
    const period = getFinancialPeriod(new Date(Date.UTC(2026, 7, 27))); // 27 Aug 2026
    expect(period.start).toEqual(new Date(Date.UTC(2026, 7, 27, 0, 0, 0, 0)));
    expect(period.end).toEqual(new Date(Date.UTC(2026, 8, 26, 23, 59, 59, 999)));
    expect(period.key).toBe("2026-08-27_2026-09-26");
  });

  it("puts the 26th in the period that STARTED the previous month", () => {
    const period = getFinancialPeriod(new Date(Date.UTC(2026, 8, 26))); // 26 Sep 2026
    expect(period.start).toEqual(new Date(Date.UTC(2026, 7, 27, 0, 0, 0, 0)));
    expect(period.end).toEqual(new Date(Date.UTC(2026, 8, 26, 23, 59, 59, 999)));
  });

  it("puts a mid-period date (e.g. 5 Sep) in the Aug 27 -> Sep 26 period", () => {
    const period = getFinancialPeriod(new Date(Date.UTC(2026, 8, 5))); // 5 Sep 2026
    expect(period.key).toBe("2026-08-27_2026-09-26");
  });

  it("rolls over the year correctly (December -> January)", () => {
    const period = getFinancialPeriod(new Date(Date.UTC(2026, 11, 30))); // 30 Dec 2026
    expect(period.start).toEqual(new Date(Date.UTC(2026, 11, 27, 0, 0, 0, 0)));
    expect(period.end).toEqual(new Date(Date.UTC(2027, 0, 26, 23, 59, 59, 999)));
  });
});

describe("isWithinPeriod", () => {
  it("includes both boundary dates", () => {
    const period = getFinancialPeriod(new Date(Date.UTC(2026, 7, 27)));
  ...
  it("excludes a date from the following period", () => {
    const period = getFinancialPeriod(new Date(Date.UTC(2026, 7, 27)));
    expect(isWithinPeriod(new Date(Date.UTC(2026, 8, 27)), period)).toBe(false);
  });
});

describe("getRecentPeriods", () => {
  it("returns N consecutive, non-overlapping periods, most recent first", () => {
    const periods = getRecentPeriods(3, new Date(Date.UTC(2026, 8, 5))); // 5 Sep 2026
